definePageMeta<template>
  <main class="w-full grid md:grid-cols-12 lg:grid-cols-12 xl:grid-cols-12 2xl:grid-cols-12 grid-cols-1 md:h-[100vh] lg:h-[100vh] xl:h-[100vh] 2xl:h-[100vh] md:max-h-[100vh] lg:max-h-[100vh] xl:max-h-[100vh] 2xl:max-h-[100vh] ">
    <div class="md:col-span-7 lg:col-span-7 xl:col-span-7 2xl:col-span-8 col-span-1 md:block lg:block xl:block 2xl:block hidden">
      <img class="w-full h-[100vh] object-cover" src="/login.png" alt="logo">
    </div>
    <div  
      class="relative md:bg-[#2679b2] lg:bg-[#2679b2]  xl:bg-[#2679b2] md:bg-none lg:bg-none xl:bg-none bg-cover bg-center bg-[#0000004b] bg-opacity-50 bg-blend-multiply bg-[url(/login.png)] md:col-span-5 lg:col-span-5 xl:col-span-5 2xl:col-span-4  col-span-1 flex flex-col justify-around p-6 md:h-auto lg:h-auto xl:h-auto h-[100vh]">
        ...
        <div class="absolute left-0 top-0 p-10">
          <button class="border-2 px-4 py-1 bg-white border-white cursor-pointer bg-primary-50" @click="setLocale(locale === 'en'? 'km': 'en')">
            <img 
              v-if="locale==='en'" 
              width="30"  
              src="https://cdn-icons-png.flaticon.com/512/14009/14009894.png" 
              alt="km">
            <img 
              v-else 
              width="30"  
      ...
      <template v-if="loginIndex!==0">
          <div class="w-full flex ">
            <button 
              @click="loginIndex = 0" 
              class="px-4 py-1 border rounded-sm cursor-pointer bg-white hover:bg-[#2679b2] hover:text-white border-white">
                <svg 
      ...
      <template v-if="loginIndex===0">
        <div class="w-full flex flex-col gap-6 max-h-24">
          <div class="flex justify-center">
            <img 
              width="100" 
              src="https://bdc.mef.gov.kh/wp-content/uploads/2024/03/tsc-1.png" 
              alt="logo">
          </div>
          <h1 class="text-center md:text-white lg:text-white xl:text-white 2xl:text-white text-white text-2xl">{{ $t('welcome') }}</h1>
        </div>
        <div class="px-8 flex flex-col gap-3">
            <button 
              @click="loginCamdigiKeyAction" 
              ...
              <span class="flex gap-x-3 items-center">
                <img 
                  width="30" 
                  src="https://play-lh.googleusercontent.com/PQSrgxYYNFUt2GO30pTfzg21u_xEvDJSCrspHgap-XkeqBhyPltrevVnMrvVsVa-f3I" 
                  alt="camdi-logo">
              ...
              @submit.prevent="onSubmit" 
                ...
                <div class="mb-1 flex flex-col gap-6">
                  <div class="w-full max-w-sm min-w-[200px]">
                    ...
                    <span class="text-left text-sm/tight text-red-400" v-if="errors.name">{{ errors.name ==='Required' ? $t('required') : errors.name }}</span>
                  </div>
                  <div class="w-full max-w-sm min-w-[200px]">
                    ...
                    <Field 
                      ...
                      <span class="text-left text-sm/tight text-red-400" v-if="errors.email">{{ errors.email ==='Required' ? $t('required') : errors.email }}</span>
                  </div>
                  <div class="w-full max-w-sm min-w-[200px]">
                    ...
                    <Field 
                      ...
                      <span class="text-left text-sm/tight text-red-400" v-if="errors.password">{{ errors.password ==='Required' ? $t('required') : errors.password }}</span>
                  </div>
                </div>
                ...
                <button 
                  type="submit" 
                  class="mt-4 w-full cursor-pointer rounded-md bg-[#2679b2] py-2 px-4 border border-transparent text-center text-sm text-white transition-all shadow-md hover:shadow-lg focus:bg-[#2679b2] focus:shadow-none active:bg-[#2679b2] hover:bg-[#2679b2] active:shadow-none">
                  {{ $t('signin') }}
                </button>
...
<script setup lang="ts">
  import { Form, Field, ErrorMessage, useForm, useField, configure } from 'vee-validate';
  ...
  const validationSchemaT = () => z.object({
  ...
  const validationSchema = ref(toTypedSchema(validationSchemaT()))

    watch(locale, () => {
      console.log('Changed...',locale.value)
        validationSchema.value = toTypedSchema(validationSchemaT())
    });
  ...
  //   configure({
  //   generateMessage: (field, rule, data) => {
  //   // Custom error message generation logic
  //     return `Custom error message for ${field} `;
  //   },
